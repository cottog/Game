...
void Map::addMonster(int x, int y) {
	...
	if (rng->getInt(0,100) < 80) {
		//create an orc
		Actor *orc = new Actor(x,y,'h',"infected crewmember",TCODColor::desaturatedGreen);
		...
		//create a troll
		Actor *troll = new Actor(x,y,'T',"gross alien",TCODColor::darkBlue);
		...
		for (int y = 0; y < height; y++) {
			if (isInFov(x,y)) {
				...
				if (isWall(x,y)) {
					...
					engine.mapcon->setChar(x, y, ' ');
					engine.mapcon->setCharBackground(x,y,TCODColor::darkerGrey);
				}
			}
		}
	}
